...
```{r}
MA_E_fb <- MA_E_fb %>%
  mutate(pct_fb = fb_popE / tot_popE) %>%
  filter(tot_popE > 0)
```
...
ggplot(MA_E_fb) +
  ...
  geom_sf(color = NA, aes(fill = pct_fb), alpha = 0.7) +
  coord_sf(crs = MA_state_plane) +
  scale_fill_continuous(low="cornsilk", high="darkgreen", 
                       na.value=NA,
                       name = "Percent of Suffolk and Middlesex County residents\nidentifying as foreign born",
...
MA_E_palette <- colorNumeric(c("Greens"),
                               MA_E_fb$pct_fb)
...
MA_E_fb_map <- leaflet(MA_E_fb) %>%
  ...
  addPolygons(fillColor = ~MA_E_palette(pct_fb), weight = 1, color = "gray", fillOpacity = 0.7,
              ...
              popup = paste("Total residents: ", MA_E_fb$tot_popE, "<br/>",
                           ...
                           MA_E_fb$pct_fb, "%)", sep = "")) %>%
    addLegend("bottomright", pal = MA_E_palette, values = ~pct_fb,
